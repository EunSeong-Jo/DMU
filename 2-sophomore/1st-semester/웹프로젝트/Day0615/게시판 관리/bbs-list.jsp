...
<!-- 공통 메뉴 삽입 -->
<jsp:include page="../include_menu.jsp"/>  // 📝 JSP include 지시어 (`<jsp:include ...>`) → 공통 메뉴 삽입 → 시험 출제 가능
    ...
    <%
        ...
        List<BoardDto> boardList = boardService.getBoardList(); // DB에서 게시글 목록 조회  // 📝 게시글 목록 조회 메서드 호출 (`getBoardList()`) → 시험 출제 가능

        int totalCount = boardList.size(); // 전체 글 수
        int i = 0;

        // 게시글 리스트 반복 출력
        for (BoardDto board : boardList) {  // 📝 향상된 for문 (`for (BoardDto board : boardList)`) → 시험 출제 가능
    %>
    <tr>
    ...
    <!-- 제목: 클릭하면 수정 페이지로 이동 -->
    <td>
    <a href="bbs-edit.jsp?id=<%=board.getId()%>">  // 📝 수정 페이지 링크에 게시글 ID 전달 (`bbs-edit.jsp?id=...`) → 시험 출제 가능
    <%= board.getSubject() %>
    </a>
    </td>
    ...
    <td>
    <button
    onclick="return confirm('삭제하시겠습니까?') ? location.href='board-delete.jsp?id=<%=board.getId()%>' : false;"  // 📝 삭제 버튼 클릭 시 ID를 쿼리스트링으로 전달 → 시험 출제 가능
    type="button">
    삭제
    </button>
    </td>
